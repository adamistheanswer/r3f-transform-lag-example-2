...
const buildBlockReadings = (item) => {
  const matrix = new Array(item.horizontal)
    .fill()
    .map(() => new Array(item.vertical).fill(0));
  for (var i = 0; i < matrix.length; i++) {
    for (var j = 0; j < matrix[i].length; j++) {
      const depthArr = [...Array(item.count)].map((_, idx) => {
        return {
          ...item,
          y: item.y + item.height * j,
          x: item.x + item.width * i,
          z: item.z + item.depth * idx
        };
      });
      matrix[i][j] = depthArr;
    }
...
export const generateBlocksFromData = (row, YAdd, XSub, depth, orbit) => {
  ...
  return allReadings?.map((blockGroup, i) => (
    <WrappedBlock
      key={i + blockGroup[0].depth}
